Module.register("MMM-Background", {
	defaults: {
		...
		addBackgroundFade: [], // adds fades for the top and bottom bar regions (leave an empty list to remove fades)
        clearCacheOnStart: false, // clear Electron's cache on MM start
	...
	fetchPhoto: function() {
		...
		req.addEventListener("load", function() {
			...
			if (this.status === 200) {
				self.processPhoto(unsplashData);
			} else if ("errors" in unsplashData) {
				self.processError(`The Unsplash API returned the error "${unsplashData["errors"].join(", ")}"`);
			} else {
				self.processError(`Unsplash Error: ${this.status}, ${this.statusText}`);
		...
		req.addEventListener("error", function() {
			...
			self.processError("Could not connect to the Unsplash server.");
		});
...
/*    getDom: function() {
        ...
        if (self.photoDAta !== null) {
          var img = self.img;   
          img.style.filter = self.config.filter;
          img.src = self.getImageUrl(self.img.src);
          img.onload = function() {
            img.style.opacity = self.getOpacity(img);
        ...
        setBackgroundTint(self.img.dark);
        return wrapper;
      },

      getOpacity: function(image) {
        var self = this;
        
    
        if (self.img.isLight) {
          return self.config.brightImageOpacity;
        } else {
          return self.config.backgroundOpacity;
        }
      },*/
